...
python -m EasyLM.models.llama.convert_easylm_to_hf --load_checkpoint=params::${MODEL_PATH} --tokenizer_path='gs://hamishi-dev/easylm/llama/tokenizer.model' --model_size=${MODEL_SIZE} --output_dir=tmp

beaker dataset upload tmp --name ${MODEL_NAME} --workspace ${WORKSPACE} &> tmp.log
